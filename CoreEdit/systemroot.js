...
systemroot.prototype = {
    ...
    startApp: function () {
        ...
        $(document).delegate(".refeshtrangchu", "click", function () {
            ...
            localStorage.removeItem('strChucNang_Id');
            localStorage.removeItem("strChucNang_Id");
            console.log(localStorage.getItem("strChucNang_Id"));
   ...
   */
    ...
    */
    checkChucNang: function () {
        var me = this;
        if (!me.appId) {
            var strChucNang = localStorage.getItem("strChucNang");
            if (strChucNang) {
    ...
    setUngDung: function (objUngDung) {
        ...
        me.getlistByUser_ChucNang();
        localStorage.setItem("strChucNang", JSON.stringify(objChucNang));
    },
    getlistByUser_ChucNang: function () {
        var me = this;
        me.save_KiemTraTaiKhoanCB();
        if (!me.checkChucNang()) return;
        var obj_save = {
            'action': 'CMS_Quyen_MH/DSA4BRICKTQiDyAvJhUpJC4PJjQuKAU0LyYeCCUP',
    ...
    save_KiemTraTaiKhoanCB: function () {
        var me = this;
        if (!me.objApi.checkCanBo) return;
        //--Edit
        var obj_save = {
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    if (data.Data == "CANBO") {
                        alert('Cán bộ, Giảng viên dùng vui lòng truy cập https://iu.cmcu.edu.vn/qtdh/index.aspx để sử dụng hệ thống.');
                        window.location.href = "/qtdh/login.aspx"
                    }
                }
                //else {
    ...
    */
    ...
    genHTML_MenuVertical: function (data) {
        ...
        console.log(localStorage.getItem("strChucNang_Id"));
        console.log(me.strChucNang_Id);
        var strChucNang_Id = localStorage.getItem("strChucNang_Id");
        me.triggerChucNang_Id(strChucNang_Id);
        if (!localStorage.getItem("strChucNang_Id") && window.innerWidth < 769) {
            console.log("1234");
    ...
    genPath_ChucNang: function () {
        ...
        //}
        if (typeof (Storage) !== "undefined") {
            localStorage.setItem("strChucNang_Id", data[0].ID);
        }
   ...
   -------------------------------------------*/
    ...
    getList_checkUser: function () {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'CMS_QuanLyNguoiDung_MH/DSA4FSkuLyYVKC8PJjQuKAU0LyYP',
            'func': 'pkg_chung_quanlynguoidung.LayThongTinNguoiDung',
            'iM': edu.system.iM,
            ...
            success: function (data) {
                if (data.Success) {
                    var obUser = {
                        ...
                        TAIKHOAN: data.Data[0].TAIKHOAN,
                        TAIKHOANNGUOIDUNG: data.Data[0].TAIKHOANNGUOIDUNG,
                        TENDAYDU: data.Data[0].TENDAYDU,
                        TENDONVI: data.Data[0].TENDONVI
                    };
                    localStorage.setItem("ckUser", JSON.stringify(obUser));
                    //sessionStorage.setItem("ckUser", JSON.stringify(obUser));
                }
    ...
    checkDevTool: function () {
        //if ($("#lockazz").length == 0 || localStorage.getItem("iShk")) return;
        (function () {
            "use strict";

            var LOGIN_URL = "/login.aspx";              // đổi thành URL trang đăng nhập thực tế
